...
bool ElevationMapping::readParameters()
{
  ...
  nodeHandle_.param("point_cloud_topic", pointCloudTopic_, string("/camera/depth/points"));
...
bool ElevationMapping::updateMapLocation()
{
 ...
 tf::StampedTransform tranx;
  try {
	transformListener_.waitForTransform(map_.getFrameId(),trackPointFrameId_,trackPoint.header.stamp, ros::Duration(0.1));
