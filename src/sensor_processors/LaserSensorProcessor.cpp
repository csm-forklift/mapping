...
try{ros::Time now = ros::Time::now();
     // listener.waitForTransform("/map", "/base_link",ros::Time(0), ros::Duration(0.5));
      ...
      listener.waitForTransform("/world", "/base_link",ros::Time(0), ros::Duration(0.5));
       ...
       listener.waitForTransform("/base_link", "/sensor_link",ros::Time(0), ros::Duration(0.5));
      listener.lookupTransform("/base_link", "/sensor_link",ros::Time(0), transform2);
      // listener.waitForTransform("/base_link", "/os1",ros::Time(0), ros::Duration(0.5));
      // listener.lookupTransform("/base_link", "/os1",ros::Time(0), transform2);
    }
...
bool LaserSensorProcessor::readParameters()
{
  ...
  nodeHandle_.param("sensor_processor/min_radius", sensorParameters_["min_radius"], 0.20); // standardDeviationInBeamDirection = minRadius
  nodeHandle_.param("sensor_processor/beam_angle", sensorParameters_["beam_angle"], 0.06); // standardDeviationOfBeamRadius = beamConstant + beamAngle*measurementDistance
  nodeHandle_.param("sensor_processor/beam_constant", sensorParameters_["beam_constant"], 0.15);
...
{
//ROS_INFO("ElevationMap: point cloud %i points.", static_cast<int>(pointCloud->size()));
  ...
  ROS_INFO("ElevationMap: cleanPointCloud() reduced point cloud to %i points.", static_cast<int>(pointCloud->size()));
  return true;
	...
	for (unsigned int i = 0; i < pointCloud->size(); ++i)
 	{
		...
		float varianceNormal =
				pow(sensorParameters_.at("min_radius"),2);
